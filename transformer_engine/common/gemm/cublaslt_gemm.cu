...
void cublas_gemm(const Tensor *inputA, const Tensor *inputB, Tensor *outputD,
                 ...
                 const Tensor *inputCounter, hipStream_t stream, hipblasLtHandle_t handle = nullptr);
#else // Use cublasLt
void cublas_gemm(const Tensor *inputA, const Tensor *inputB, Tensor *outputD,
                 const Tensor *inputBias, Tensor *outputPreGelu, int m, int n, int k, int lda,
...
#ifdef __HIP_PLATFORM_AMD__
void nvte_cublas_gemm_with_handle(const NVTETensor A, const NVTETensor B, NVTETensor D, const NVTETensor bias,
                      NVTETensor pre_gelu_out, bool transa, bool transb, bool grad,
                      NVTETensor workspace, bool accumulate, bool use_split_accumulator,
                      int math_sm_count, cudaStream_t stream, hipblasLtHandle_t handle) {
  NVTE_API_CALL(nvte_cublas_gemm_with_handle);
  using namespace transformer_engine;
  ...
  cublas_gemm(inputA, inputB, outputD,  biasTensor, outputGelu, m, n, k, lda, ldb, ldd,
              ...
              math_sm_count, 0, 0, false, nullptr, stream, handle);
}
#endif // __HIP_PLATFORM_AMD__
